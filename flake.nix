      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "  build-addon     - Build the ChatGPT Codex CLI Terminal add-on"
            ...
            alias build-addon='podman build --build-arg BUILD_FROM=ghcr.io/home-assistant/amd64-base:3.19 -t local/chatgpt-terminal ./chatgpt-terminal'
            alias run-addon='podman run -p 7681:7681 -v $(pwd)/config:/config local/chatgpt-terminal'
            alias validate-addon='echo "Note: Home Assistant builder validation requires HA OS environment"'
            alias lint-dockerfile='hadolint ./chatgpt-terminal/Dockerfile'
